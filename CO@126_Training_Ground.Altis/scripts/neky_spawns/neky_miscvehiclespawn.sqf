...
if (!IsNull _Laptop) then 
{
	...
	[[[_Laptop,_Name,_Array,_Firstmarker,_LastMarker,_Actions,_Global],
	{
		...
		_NEKY = (_this select 0) addAction [format ["<t color='#FF8000'>Remove %1</t>",(_this select 1)],
		{
			...
			[[[_Name,_Laptop,_FirstMarker,_LastMarker,"CIVILIAN",_Global],{ [(_This select 0), (_This select 1), (_This select 2), (_This select 3), (_This select 4), (_This select 5)] Call NEKY_fnc_HandleVehicles; }], "BIS_FNC_SPAWN", True] Call BIS_Fnc_MP;
