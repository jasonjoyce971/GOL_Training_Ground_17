...
_NEKY=	_Laptop addAction [format ["<t color='#00FF00'>Ready %1</t>",_Name], {
		...
		if (_Type == "") then {[] call compile preprocessFileLineNumbers ("Scripts\NEKY_Spawns\NEKY_Factions\Factions.sqf");};
...
_NEKY2 =	_Laptop addAction [format ["<t color='%1'>%2</t>",_Colors select _Index,_Factions select _Index], {
				...
				if (_Type == "") then {[[[_Factions select _Index, _FirstMarker, _LastMarker,_Laptop,_Name,_Global],"Scripts\NEKY_Spawns\NEKY_VehicleSpawn.sqf"], "BIS_fnc_ExecVM",false,false] call BIS_fnc_MP; NEKY_VehicleSpawn = False;};
				if (_Type == "Civilian") then {[[[_FirstMarker, _LastMarker,_Laptop,_Name,_Global],"Scripts\NEKY_Spawns\NEKY_MiscVehicleSpawn.sqf"], "BIS_fnc_ExecVM",false,false] call BIS_fnc_MP; NEKY_MiscVehicleSpawn = False;};
