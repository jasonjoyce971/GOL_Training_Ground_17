...
scopeName "main";
while {soloPlay} do // Main Task Spawner Loop -> needs to break on selecting an alternate exercise
{
    scopeName "mainloop";
    ...
    while {count ([west] call getSideActiveTasks) < count _alivePlayers} do
    {
        ...
        {
            {
                ...
                {
                    [_newLZLocation, [_plr], _side] spawn createPickupLZ;
                    _plrAssigned = true;
                    breakTo "spawnloop";
                };
        ...
        {
            [_newLZLocation, false,_side] spawn createPickupLZ;
        };
        // rate limit
        sleep 10;
    };
