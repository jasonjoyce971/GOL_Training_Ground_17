...
{
    _taskType = "attack";
    _enemies = _enemies + ([_lzLocation, _lzAA, _side] call createEnemySquads);
};
...
diag_log format["Suspected Break Point"];
diag_log format["Bind variable = %1",_bindToVehicle];
...
private _trgCond = format["((%1 in thisList) && ([%2] call isLanded))", _assignToPlayer,_bindToVehicle];
diag_log format["createDropoffLZ: _trgCond %1", _trgCond];
_trg setTriggerStatements[_trgCond , "", ""];

diag_log format["Suspected Break Point Pass"];
